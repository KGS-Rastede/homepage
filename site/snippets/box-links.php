<div class="card mb-3 mt-3">
    <div class="card-body ">
        <ul>
            <a href="https://nextcloud.kgs-rastede.logoip.de/index.php/login">
                <img src="<?= $kirby->url('assets') ?>/bilder/nextcloud.svg" class="img-fluid">
            </a>
        </ul>
        <hr>
        <ul>
            <a href="https://thalia.webuntis.com/WebUntis/?school=kgs-rastede">
                <img src="<?= $kirby->url('assets') ?>/bilder/webuntis.svg" class="img-fluid">
            </a>
        </ul>
        <hr>
        <ul>
            <a href="https://moodle30.kgs-rastede.logoip.de/">
                <img src="<?= $kirby->url('assets') ?>/bilder/moodle-kgs.svg" class="img-fluid">
            </a>
        </ul>
        <hr>
        <ul>
            <a href="https://www.schuelerzeitung.kgs-rastede.de/">
                <img src="<?= $kirby->url('assets') ?>/bilder/irregulaer.png" alt="Schülerzeitung" class="img-fluid">
            </a>
        </ul>
        <hr>
        <ul>
            <a href="<?= page('allgemeines/mensa')->url() ?>" class="btn btn-warning btn-lg btn-block" role="button" aria-pressed="true">Mensa</a>
        </ul>
        <hr>



        <ul>
            <a href="https://kgs-rastede.schulserver.de">
                <img src="<?= $kirby->url('assets') ?>/bilder/IServ_Logo_klein_RGB_clean.svg" alt="IServ" class="img-fluid">
            </a>
        </ul>
    </div>
</div>