...
<script>
    ...
    map.on('load', () => {
        ...
        // --------- Wände hinzufügen ---------
        levels.forEach(level => {
            addLayer(`room_walls`, 'line', level, ['all', ['==', 'level', level],
                ['==', 'indoor', 'room']
            ], {
                'line-color': '#222',
                'line-opacity': 0.25,
    ...
    function setTwoD() {
        ...
        map.easeTo({
            ...
            zoom: 18.7,
            duration: 1000
        }); // Setzte Neigung auf 0 für 2D Ansicht und zoome raus
    ...
    function setThreeD() {
        ...
        map.easeTo({
            ...
            zoom: 18,
            duration: 1000
        }); // Setzte Neigung auf 50 Graf für 3D sicht und zoome rein
    ...
    function toggleFloor() {
        ...
        const featuresTemp = map.querySourceFeatures('floorplan', {
            sourceLayer: 'room_searched'
        });
        ...
        return geojson = {
            ...
            'features': [
                <?php foreach ($block->reise()->toBlocks() as $block) : ?> {
                        ...
                        'properties': {
                            <?php
                            if ($block->name()->value() == "Feldbreite") : ?> 'message': '<?= $page->adresse_s2()->toBlocks()->first()->content()->text()->value() ?>', //Holt Die Beschreibung des Icons
                                'iconSize': [50, 50],


                            <?php elseif ($block->name()->value() == "Hauptgebäude") : ?> 'message': '<?= $page->adresse_s1()->toBlocks()->first()->content()->text()->value() ?>', //Holt Die Beschreibung des Icons
                                'iconSize': [50, 50],
